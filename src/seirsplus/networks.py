...
def generate_K5_school_contact_network(num_grades, num_classrooms_per_grade, class_sizes, 
    ...
    schoolNetwork = scipy.sparse.block_diag([networkx.adjacency_matrix(studentNetwork), networkx.adjacency_matrix(teacherstaffNetwork)])
...
def generate_highschool_contact_network(num_grades=4, num_students_per_grade=200, num_communities_per_grade=20,
                                        student_mean_intragrade_degree=16, student_pct_contacts_intergrade=0.2,
                                        num_student_blocks=2, block_by_household=True, connect_students_in_households=True, 
                                        num_teachers=125, num_staff=75, num_teacher_staff_communities=10, teacher_staff_degree=12,
    ...
    for g, gradeNetwork in enumerate(gradeNetworks):
        ...
        for i in list(range(gradeNetwork.number_of_nodes())):
            ...
            if(len(gradeNetworks) > 1):
                for d in list(range(i_interCohortDegree)):
                    j = numpy.random.choice(list(range(0, gradeStartIdx))+list(range(gradeFinalIdx+1, N)))
    ...
    # Combine the teacher/staff network block with the student network block:
    highschoolAdjMatrix = scipy.sparse.block_diag([networkx.adjacency_matrix(studentNetwork), networkx.adjacency_matrix(teacherstaffNetwork)])
    ...
    for studentID in studentIDs:
        ...
        for c in range(num_classes_per_student):
            ...
            classLevel          = numpy.random.choice(list(range(num_grades)), p=classlevel_probs[studentGradeLevel])
    ...
    if(connect_students_in_households):
        ...
        while(len(studentsNeedingHousehold) > 0):
            ...
            numHSHousemates = min( numpy.random.choice([0, 1, 2], p=[0.71, 0.25, 0.04]), len(studentsNeedingHousehold) )
    ...
    networks['offsite-all'] = networkx.classes.function.create_empty_copy(highschoolNetwork)
