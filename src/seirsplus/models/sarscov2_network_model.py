...
class SARSCoV2NetworkModel(CompartmentNetworkModel):
    ...
    def __init__(self, networks,
        ...
        latent_period         = distributions.gamma_dist(mean=3.0, coeffvar=0.6, N=self.pop_size) if latent_period=='default'         else np.array(latent_period).reshape((1,self.pop_size)) if isinstance(latent_period, (list, np.ndarray)) else np.full(fill_value=latent_period, shape=(1,self.pop_size))
        presymptomatic_period = distributions.gamma_dist(mean=2.2, coeffvar=0.5, N=self.pop_size) if presymptomatic_period=='default' else np.array(presymptomatic_period).reshape((1,self.pop_size)) if isinstance(presymptomatic_period, (list, np.ndarray)) else np.full(fill_value=presymptomatic_period, shape=(1,self.pop_size))
        symptomatic_period    = distributions.gamma_dist(mean=4.0, coeffvar=0.4, N=self.pop_size) if symptomatic_period=='default'    else np.array(symptomatic_period).reshape((1,self.pop_size)) if isinstance(symptomatic_period, (list, np.ndarray)) else np.full(fill_value=symptomatic_period, shape=(1,self.pop_size))
