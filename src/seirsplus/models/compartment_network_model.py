...
class CompartmentNetworkModel:
    def __init__(
        ...
        self.stateID = {}
        self.default_state = list(self.compartments.keys())[
            0
        ]  # default to first compartment specified
        ...
        for c, compartment in enumerate(self.compartments):
            ...
            if (
                "exclude_from_eff_pop" in comp_params
                and comp_params["exclude_from_eff_pop"] == True
            ):
        ...
        if node_groups:
            ...
            for groupName, nodeList in node_groups.items():
                self.nodeGroupData[groupName] = {
                    ...
                    "mask": np.in1d(range(self.pop_size), nodeList).reshape(
                        (self.pop_size, 1)
                    ),
                }
                for compartment in self.compartments:
                    self.nodeGroupData[groupName][compartment] = np.zeros(
                        self.pop_size * min(len(self.compartments), 10)
                    )
                    self.nodeGroupData[groupName][compartment][0] = np.count_nonzero(
    ...
    def update_networks(self, new_networks):
        if not isinstance(new_networks, dict):
            raise BaseException(
                "Specify networks with a dictionary of adjacency matrices or networkx objects."
            )
        else:
            ...
            for netID, network in new_networks.items():
                if type(network) == np.ndarray:
                    new_networks[netID] = {
                        "networkx": networkx.from_numpy_matrix(network),
                    ...
                    raise BaseException(
                        "Network",
                        netID,
                        "should be specified by an adjacency matrix or networkx object.",
                    )
                # Store the number of nodes and node degrees for each network:
                new_networks[netID]["num_nodes"] = int(
                    new_networks[netID]["adj_matrix"].shape[1]
                )
                new_networks[netID]["degree"] = (
                    new_networks[netID]["adj_matrix"]
                    .sum(axis=0)
                    .reshape(new_networks[netID]["num_nodes"], 1)
                )
                # Set all individuals to be active participants in this network by default:
                new_networks[netID]["active"] = np.ones(
                    new_networks[netID]["num_nodes"]
                )
                # Set all individuals to be inactive in this network when in isolation by default:
                new_networks[netID]["active_isolation"] = np.zeros(
                    new_networks[netID]["num_nodes"]
                )
    ...
    def update_compartments(self, new_compartments):
        ...
        def reshape_param_vals(nested_dict):
            for key, value in nested_dict.items():
                ...
                # Convert all other parameter values to arrays corresponding to the population size:
                    ...
                    nested_dict[key] = (
                        np.array(value).reshape((self.pop_size, 1))
                        if isinstance(value, (list, np.ndarray))
                        else np.full(fill_value=value, shape=(self.pop_size, 1))
                    )
        ...
        for compartment, comp_dict in new_compartments.items():
            ...
            if "local_transm_offset_mode" not in transm_dict:
                transm_dict[
                    "local_transm_offset_mode"
                ] = "none"  # default when not provided
            ...
            for G in self.networks:
                ...
                # M_G = (AB)_G * D_G
                self.infectivity_mat[compartment][G] = scipy.sparse.csr_matrix.multiply(
                    transm_dict[G], transm_dict["offsets"][G]
                )

            if "exogenous" not in transm_dict or not isinstance(
                transm_dict["exogenous"], (int, float)
            ):
                transm_dict["exogenous"] = 0.0

            if "global" not in transm_dict or not isinstance(
                transm_dict["global"], (int, float)
            ):
                transm_dict["global"] = np.sum(
                    [np.sum(transm_dict[G][transm_dict[G] != 0]) for G in self.networks]
                ) / max(
                    np.sum([transm_dict[G].count_nonzero() for G in self.networks]), 1
                )

        # Check the initial and exogenous_prevalence params for each compartment,
        # defaulting to 0 when missing or invalid:
        for compartment, comp_dict in new_compartments.items():
            if "initial_prevalence" not in comp_dict or not isinstance(
                comp_dict["initial_prevalence"], (int, float)
            ):
                comp_dict["initial_prevalence"] = 0.0
            if "exogenous_prevalence" not in comp_dict or not isinstance(
                comp_dict["exogenous_prevalence"], (int, float)
            ):
        ...
        for compartment, comp_params in self.compartments.items():
            ...
            for destState, transition_params in comp_params["transitions"].items():
                if destState not in self.compartments:
                    print(
                        "Destination state", destState, "is not a defined compartment."
                    )
                    continue

                if self.transition_mode == "time_in_state":
                    propensity_temporal_transition = (
                        (
                            self.transition_timer_wt
                            * (
                                np.greater(self.state_timer, transition_params["time"])
                                & (self.X == self.stateID[compartment])
                            )
                            * transition_params["active_path"]
                        )
                        if any(transition_params["time"])
                        else np.zeros_like(self.X)
                    )

                else:  # exponential_rates
                    propensity_temporal_transition = (
                        transition_params["rate"]
                        * (self.X == self.stateID[compartment])
                        * transition_params["active_path"]
                    )
                ...
                transitions.append(
                    {"from": compartment, "to": destState, "type": "temporal"}
                )
            ...
            for infectiousState, susc_params in comp_params["susceptibilities"].items():
                ...
                if infectiousState not in propensity_infection_local:
                    ...
                    propensity_infection_local[infectiousState] = np.zeros(
                        (self.pop_size, 1)
                    )

                    denom_numContacts = np.zeros((self.pop_size, 1))

                    # Get the number of contacts relevant for the local transmission
                    # denominator for each individual:
                    for netID, G in self.networks.items():
                        bool_isGactive = (
                            ((G["active"] != 0) & (self.isolation == 0))
                            | ((G["active_isolation"] != 0) & (self.isolation != 0))
                        ).flatten()
                        denom_numContacts += (
                            G["adj_matrix"][
                                :, np.argwhere(bool_isGactive).flatten()
                            ].sum(axis=1)
                            if self.local_trans_denom_mode == "active_contacts"
                            else G["degree"]
                        )
                    ...
                    for netID, G in self.networks.items():
                        ...
                        bool_hasGactiveInfectiousContacts = np.asarray(
                            scipy.sparse.csr_matrix.dot(
                                M, scipy.sparse.diags(bin_isGactive)
                            )[:, j_isInfectious]
                            .sum(axis=1)
                            .astype(bool)
                        ).flatten()
                        ...
                        bool_isInfectible = (
                            bool_isGactive & bool_hasGactiveInfectiousContacts
                        )
                        ...
                        propensity_infection_local[infectiousState][
                            i_isInfectible
                        ] += np.divide(
                            scipy.sparse.csr_matrix.dot(
                                M[i_isInfectible, :][:, j_isInfectious],
                                (self.X == self.stateID[infectiousState])[
                                    j_isInfectious
                                ],
                            ),
                            denom_numContacts[i_isInfectible],
                            out=np.zeros_like(
                                propensity_infection_local[infectiousState][
                                    i_isInfectible
                                ]
                            ),
                            where=denom_numContacts[i_isInfectible] != 0,
                        )
                ...
                propensity_infection = (self.X == self.stateID[compartment]) * (
                    ...
                    * (
                        (self.openness)
                        * (
                            transm_params["exogenous"]
                            * self.exogenous_prevalence[compartment]
                        )
                        + (1 - self.openness)
                        * (
                            (self.mixedness)
                            * (
                                (
                                    transm_params["global"]
                                    * self.counts[infectiousState][self.tidx]
                                )
                                / self.N[self.tidx]
                            )
                            + (1 - self.mixedness)
                ...
                for destState, transition_params in susc_params["transitions"].items():
                    ...
                    propensity_infection_transition = (
                        propensity_infection * transition_params["active_path"]
                    )

                    propensities.append(propensity_infection_transition)
                    transitions.append(
                        {"from": compartment, "to": destState, "type": "infection"}
                    )

        propensities = (
            np.hstack(propensities) if len(propensities) > 0 else np.array([[]])
        )
        ...
        if propensities.sum() > 0:
            ...
            if tau > max_dt:
                ...
                if self.isolation_period is not None:
                    i_exitingIsolation = np.argwhere(
                        self.isolation_timer >= self.isolation_period
                    ).flatten()
            ...
            assert self.X[transitionNode] == self.stateID[transition["from"]], (
            ...
            self.set_state(
                transitionNode, transition["to"]
            )  # self.X[transitionNode] = self.stateID[transition['to']]
            ...
            if self.log_infection_info and transition["type"] == "infection":
                ...
                for netID, G in self.networks.items():
                    infectionLog["contact_info"].update(
                        {
                            "num_contacts": G["degree"][transitionNode, 0],
                            "contacts_states": self.X[
                                np.argwhere(G["adj_matrix"][transitionNode, :] > 0)
                            ]
                            .flatten()
                            .tolist(),
                            "contacts_isolations": self.isolation[
                                np.argwhere(G["adj_matrix"][transitionNode, :] > 0)
                            ]
                            .flatten()
                            .tolist(),
    ...
    def run(self, T, checkpoints=None, max_dt=None, default_dt=0.1):
        ...
        # TODO: Pre-process checkpoint values:
        ...
        while running:
            ...
            # TODO: Handle checkpoints if applicable:
    ...
    def update_data_series(self):
        ...
        for compartment in self.compartments:
            self.counts[compartment][self.tidx] = np.count_nonzero(
                self.X == self.stateID[compartment]
            )
        ...
        if self.nodeGroupData:
            for groupName in self.nodeGroupData:
                for compartment in self.compartments:
                    self.nodeGroupData[groupName][compartment][
                        self.tidx
                    ] = np.count_nonzero(
                        self.nodeGroupData[groupName]["mask"] * self.X
                        == self.stateID[compartment]
                    )
                    # ------------------------------------
                    if compartment not in self.excludeFromEffPopSize:
                        self.nodeGroupData[groupName]["N"][self.tidx] += self.counts[
                            compartment
                        ][self.tidx]
    ...
    def increase_data_series_length(self):
        ...
        self.tseries = np.pad(
            self.tseries,
            [(0, self.pop_size * min(len(self.compartments), 10))],
            mode="constant",
            constant_values=0,
        )
        ...
        for compartment in self.compartments:
            self.counts[compartment] = np.pad(
                self.counts[compartment],
                [(0, self.pop_size * min(len(self.compartments), 10))],
                mode="constant",
                constant_values=0,
            )
        ...
        if self.nodeGroupData:
            for groupName in self.nodeGroupData:
                for compartment in self.compartments:
                    self.nodeGroupData[groupName][compartment] = np.pad(
                        self.nodeGroupData[groupName][compartment],
                        [(0, self.pop_size * min(len(self.compartments), 10))],
                        mode="constant",
                        constant_values=0,
                ...
                self.nodeGroupData[groupName]["N"] = np.pad(
    ...
    def finalize_data_series(self):
        ...
        for compartment in self.compartments:
            self.counts[compartment] = np.array(self.counts[compartment], dtype=float)[
                : self.tidx + 1
            ]
        ...
        if self.nodeGroupData:
            for groupName in self.nodeGroupData:
                for compartment in self.compartments:
                    self.nodeGroupData[groupName][compartment] = np.array(
                        self.nodeGroupData[groupName][compartment], dtype=float
                    )[: self.tidx + 1]

                self.nodeGroupData[groupName]["N"] = np.array(
    ...
    def process_transition_probs(self, transn_dict):
        ...
        for poststate in poststates:
            try:
                ...
                prob = (
                    np.array(prob).reshape((self.pop_size, 1))
                    if isinstance(prob, (list, np.ndarray))
                    else np.full(fill_value=prob, shape=(self.pop_size, 1))
                )
                ...
                if len(poststates) == 1:
                    ...
                    print(
                        "Multiple transitions specified, but not all probabilities provided: Assuming equiprobable."
                    )
                    transn_dict[poststate]["prob"] = np.full(
                        1 / len(poststates), shape=(self.pop_size, 1)
                    )
                    probs.append(transn_dict[poststate]["prob"])
        probs = np.array(probs).reshape((len(poststates), self.pop_size))
        # ----------------------------------------
        rands = [
            poststates[np.random.choice(len(poststates), p=probs[:, i])]
            for i in range(self.pop_size)
        ]
        # ----------------------------------------
        for poststate in transn_dict:
            transn_dict[poststate]["active_path"] = np.array(
                [1 if rands[i] == poststate else 0 for i in range(self.pop_size)]
            ).reshape((self.pop_size, 1))
    ...
    def process_local_transmissibility(self, transm_dict, network):
        ...
        try:
            ...
            local_transm_vals = (
                np.array(local_transm_vals)
                if isinstance(local_transm_vals, (list, np.ndarray))
                else np.full(fill_value=local_transm_vals, shape=(self.pop_size, 1))
            )
            # ----------------------------------------
            # Generate matrix of pairwise transmissibility values:
            if (
                local_transm_vals.ndim == 2
                and local_transm_vals.shape[0] == self.pop_size
                and local_transm_vals.shape[1] == self.pop_size
            ):
                net_transm_mat = local_transm_vals
            elif (
                local_transm_vals.ndim == 1
                and local_transm_vals.shape[0] == self.pop_size
            ) or (
                local_transm_vals.ndim == 2
                and (
                    local_transm_vals.shape[0] == self.pop_size
                    or local_transm_vals.shape[1] == self.pop_size
                )
            ):
                ...
                A_beta_pairwise_byInfected = scipy.sparse.csr_matrix.multiply(
                    self.networks[network]["adj_matrix"], local_transm_vals.T
                ).tocsr()
                A_beta_pairwise_byInfectee = scipy.sparse.csr_matrix.multiply(
                    self.networks[network]["adj_matrix"], local_transm_vals
                ).tocsr()
                # ------------------------------
                ...
                if transm_dict["pairwise_mode"].lower() == "infected":
                    ...
                    net_transm_mat = scipy.sparse.csr_matrix.minimum(
                        A_beta_pairwise_byInfected, A_beta_pairwise_byInfectee
                    )
                elif transm_dict["pairwise_mode"].lower() == "max":
                    net_transm_mat = scipy.sparse.csr_matrix.maximum(
                        A_beta_pairwise_byInfected, A_beta_pairwise_byInfectee
                    )
                elif (
                    transm_dict["pairwise_mode"].lower() == "mean"
                    or transm_dict["pairwise_mode"] is None
                ):
                    net_transm_mat = (
                        A_beta_pairwise_byInfected + A_beta_pairwise_byInfectee
                    ) / 2
                    ...
                    raise BaseException(
                        "Unrecognized pairwise_mode value (support for 'infected', 'infectee', 'min', 'max', and 'mean')."
                    )
            else:
                raise BaseException(
                    "Invalid data type/shape for transmissibility values."
                )
            # ----------------------------------------
            ...
            transm_dict[network] = scipy.sparse.csr_matrix(
                np.zeros(shape=(self.pop_size, self.pop_size))
            )
    ...
    def process_local_transm_offsets(self, transm_dict, network):
        ...
        try:
            ...
            omega_vals = (
                np.array(omega_vals)
                if isinstance(omega_vals, (list, np.ndarray))
                else np.full(fill_value=omega_vals, shape=(self.pop_size, 1))
            )
            # ----------------------------------------
            # Store 2d np matrix of pairwise omega values:
            if (
                omega_vals.ndim == 2
                and omega_vals.shape[0] == self.pop_size
                and omega_vals.shape[1] == self.pop_size
            ):
                nested_dic[G + "_omega"] = omega_vals
            else:
                raise BaseException(
                    "Explicit omega values should be specified as an NxN 2d array. Else leave unspecified and omega values will be automatically calculated according to local_transm_offset_mode."
                )
        except KeyError:
            # ----------------------------------------
            ...
            if transm_dict["local_transm_offset_mode"].lower() == "pairwise_log":
                with np.errstate(
                    ...
                    omega = np.log(
                        np.maximum(self.networks[network]["degree"], 2)
                    ) / np.log(np.mean(self.networks[network]["degree"]))
                    omega[np.isneginf(omega)] = 0.0
            elif transm_dict["local_transm_offset_mode"].lower() == "pairwise_linear":
                omega = np.maximum(self.networks[network]["degree"], 2) / np.mean(
                    self.networks[network]["degree"]
                )
            elif transm_dict["local_transm_offset_mode"].lower() == "none":
                omega = np.ones(shape=(self.pop_size, self.pop_size))
            else:
                raise BaseException(
                    "Unrecognized local_transm_offset_mode value (support for 'pairwise_log', 'pairwise_linear', and 'none')."
                )
            omega_pairwise_byInfected = scipy.sparse.csr_matrix.multiply(
                self.networks[network]["adj_matrix"], omega.T
            ).tocsr()
            omega_pairwise_byInfectee = scipy.sparse.csr_matrix.multiply(
                self.networks[network]["adj_matrix"], omega
            ).tocsr()
    ...
    def process_initial_states(self):
        ...
        for c, compartment in enumerate(self.compartments):
            ...
            self.counts[compartment] = np.zeros(
                self.pop_size * min(len(self.compartments), 10)
            )
            # ----------------------------------------
            # Set initial counts for each compartment:
            if comp_params["initial_prevalence"] > 0:
                self.counts[compartment][0] = min(
                    max(int(self.pop_size * comp_params["initial_prevalence"]), 1),
                    self.pop_size - initCountTotal,
                )
        ...
        if initCountTotal < self.pop_size:
            if self.default_state is not None:
                ...
                raise BaseException(
                    "A default compartment must be designated ('default_state':True in config) when the total initial count is less than the population size."
                )
        ...
        self.X = np.concatenate(
            [
                [self.stateID[comp]] * int(self.counts[comp][0])
        ...
        if self.store_Xseries:
            self.Xseries = np.zeros(
                shape=(6 * self.pop_size, self.pop_size), dtype="uint8"
            )
        ...
        if self.track_flag_counts:
            for flag in self.allCompartmentFlags.union(self.allNodeFlags):
                ...
                self.flag_counts[flag] = np.zeros(
                    self.pop_size * min(len(self.compartments), 10)
                )
                # ----------------------------------------
    ...
    def set_state(self, node, state):
        ...
        for i in nodes:
            if state in self.compartments:
                ...
                print(
                    "Unrecognized state, " + str(state) + ". No state update performed."
                )
                return
        self.update_data_series()
    ...
    def set_transmissibility(self, compartment, transm_mode, transmissibility):
        ...
        for compartment in compartments:
            ...
            for transmMode in transmModes:
                ...
                if transmMode in self.networks:
                    ...
                    self.infectivity_mat[compartment][
                        ...
                        transm_dict[transmMode], transm_dict["offsets"][transmMode]
                    )
                # ----------------------------------------
                # Handle update to exogenous transmissibility:
                elif transmMode == "exogenous":
                    transm_dict["exogenous"] = np.array(transmissibility).reshape(
                        (self.pop_size, 1)
                    )
                ...
                # ----------------------------------------
                    ...
                    print(
                        "Transmission mode,",
                        transmMode,
                        "not recognized (expected 'exogenous', or network name in "
                        + str(list(self.networks.keys()))
                        + "); no update.",
                    )
            # ----------------------------------------
            ...
            transm_dict["global"] = np.sum(
                [np.sum(transm_dict[G][transm_dict[G] != 0]) for G in self.networks]
            ) / max(np.sum([transm_dict[G].count_nonzero() for G in self.networks]), 1)
    ...
    def set_exclude_from_eff_pop(self, compartment, exclude=True):
        ...
        for compartment in compartments:
            ...
            if exclude and not compartment in self.excludeFromEffPopSize:
                ...
                self.excludeFromEffPopSize = [
                    c for c in self.excludeFromEffPopSize if c != compartment
                ]  # remove all occurrences of compartment
    ...
    def set_network_activity(
        self, network, node="all", active=None, active_isolation=None
    ):
        nodes = (
        ...
        for i in nodes:
            for G in networks:
                ...
                if active_isolation is not None:
                    self.networks[G]["active_isolation"][i] = (
                        1 if active_isolation else 0
                    )
    ...
    def add_node_flag(self, node, flag):
        ...
        for node in nodes:
            for flag in flags:
                ...
                if flag not in self.flag_counts:
                    self.flag_counts[flag] = np.zeros_like(
                        self.counts[list(self.counts.keys())[0]]
                    )
                    self.update_data_series()
    ...
    def remove_node_flag(self, node, flag):
        ...
        for node in nodes:
            for flag in flags:
                self.node_flags = [
                    f for f in self.node_flags if f != flag
                ]  # remove all occurrences of flag
    ...
    def introduce_random_exposures(self, num, compartment="all", exposed_to="any"):
        ...
        infectiousStates = (
            ...
            if (
                not isinstance(exposed_to, (list, np.ndarray))
                and exposed_to is not None
            )
            else exposed_to
        )
        ...
        for exposure in range(num):
            for compartment in compartments:
                for infectiousState in infectiousStates:
                    if (
                        ...
                        exposure_susceptibilities.append(
                            {
                                "susc_state": compartment,
                                "inf_state": infectiousState,
                                "susceptibilities": self.compartments[compartment][
                                    "susceptibilities"
                                ][infectiousState]["susceptibility"].flatten(),
                                "mean_susceptibility": np.mean(
            ...
            exposureType = np.random.choice(
                exposure_susceptibilities,
                p=[d["mean_susceptibility"] for d in exposure_susceptibilities]
                / np.sum([d["mean_susceptibility"] for d in exposure_susceptibilities]),
            )
            exposableNodes = [
                i
                for i in range(self.pop_size)
                if self.X[i] == self.stateID[exposureType["susc_state"]]
            ]
            if len(exposableNodes) > 0:
                exposedNode = np.random.choice(
                    exposableNodes,
                    p=exposureType["susceptibilities"][exposableNodes]
                    / np.sum(exposureType["susceptibilities"][exposableNodes]),
                )
                exposedNodes.append(exposedNode)
                # --------------------
                exposureTransitions = self.compartments[exposureType["susc_state"]][
                    "susceptibilities"
                ][exposureType["inf_state"]]["transitions"]
                exposureTransitionsActiveStatuses = [
                    exposureTransitions[dest]["active_path"].flatten()[exposedNode]
                    for dest in exposureTransitions
                ]
                destState = np.random.choice(
                    list(exposureTransitions.keys()),
                    p=exposureTransitionsActiveStatuses
                    / np.sum(exposureTransitionsActiveStatuses),
                )
                # --------------------
